...
\item Ponto de corte para valores contínuos
...
\begin{Slide}{Ponto de corte para valores contínuos}
	\begin{itemize}
		\item O domínio de um atributo contínuo contém um número infinito de valores. 
		\item A estratégia usada no caso de atributos nominais não é aplicada a atributos contínuos. 
		\item A estratégia usualmente empregada é dividir a faixa de valores em duas: 
		\begin{itemize}
	...
	\begin{itemize}
		\item Para determinar o $valor$ de corte os valores do atributo contínuo são ordenados.
		\item O ponto médio entre dois valores consecutivos é um possível ponto de corte.
		\item Cada ponto de corte é avaliado com relação ao seu ganho de informação. 
		\item \textbf{Importante}: não é necessário testar todos os possíveis pontos de corte, apenas aqueles que dividem exemplos de classes diferentes.
	\end{itemize}
...
\begin{Slide}{Seleção do atributo}
	\begin{itemize}
		\item Em problemas de regressão, a função de custo a minimizar é, usualmente, o erro quadrático. 
		\item Por isso, para estimar o mérito de uma partição obtida por um teste no valor de uma variável, é utilizado a métrica SDR.
		\item Assuma um conjunto de exemplos $D$, com $n$ exemplos. A variância da variável alvo, $y$, é dada pela expressão: 
		\begin{equation}
		...
		\item Dado um teste hipotético $h_{A}$ sobre o atributo $A$, por exemplo $A \leq a_{1}$.  
		
		\item Os exemplos do conjunto $D$ serão divididos em dois subconjuntos $D_{L}$ e $D_{R}$ com tamanhos $n_{L}$ e $n_{R}$, tais que $n = n_{L} + n_{R}$. 
		
		\item A variância de $y$, a variável alvo, em cada subconjunto $D_{L}$ e $D_{R}$. é sempre menor ou igual à variância de $y$ antes da divisão. 
		
		\newpage
		
		\item Podemos estimar a redução em variância obtida pela aplicação do teste $h_{A}$:  
		
		\begin{eqnarray}
		SDR(h_{A}) = sd(D,y) - \frac{n_{L}}{n} \times sd(D_{L}, y) - \frac{n_{R}}{n} \times sd(D_{R}, y) \nonumber
		\end{eqnarray}
		
		\item Para cada atributo, e para cada possível teste no valor do atributo, é calculada a redução da variância associada a esse teste. 
		
		\item O teste que provoca uma maior redução em variância é escolhido como teste para o nó. 
		
	\end{itemize}
\end{Slide}

\begin{Slide}{Cálculo do valor do nodo de retorno}

\begin{itemize}
	\item O valor de cada nó folha recebe o valor médio dos exemplos que estão naquela folha. 
\end{itemize}	
...
\begin{Slide}{Material de \textbf{consulta}}
  \begin{itemize}
  \item Faceli et all. Inteligência Artificial: uma abordagem de Aprendizado de Máquina. Editora LTC. 2011.
