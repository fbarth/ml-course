...
\begin{Slide}{Hiperparâmetros}
	...
	\begin{itemize}
		...
		\item bootstrap = método para amostragem de exemplos (com ou sem \textit{replacement}). O default é $True$, ou seja, com \textit{replacement}. 
	\end{itemize}
\end{Slide}
...
\begin{Slide}{Implementação de RandomForest para problemas desbalanceados}
	\begin{itemize}
		\item imblearn.ensemble.BalancedRandomForestClassifier
		\item Esta implementação faz under-sample para balancear o conjunto de treinamento em cada seleção de exemplos para construção de cada uma das árvores da floresta.
	\end{itemize}
\end{Slide}

\begin{Slide}{Considerações finais}
	\begin{itemize}
		\item Em comparação com as árvores de decisão, o que se perde é a estrutura simples e interpretável; o que se ganha é a redução de \textit{overfitting}. 
	\end{itemize}
\end{Slide}
